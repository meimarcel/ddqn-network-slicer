...
class Env:
    def __init__(self, input_parameters: Dict[str, Any], processing_node_parameters: Dict[str, Any]) -> None:
        ...
        for p in processing_node_parameters:
            if p[A_TYPE].lower() == "cloud":
                self.clouds_capacity.append(p[WAVELENGTHS_CAPACITY] * p[WAVELENGTHS])
                self.clouds.append(0)
            elif p[A_TYPE].lower() == "fog":
                self.fogs_capacity.append(p[WAVELENGTHS_CAPACITY] * p[WAVELENGTHS])
                self.fogs.append(0)

    def reset(self) -> None:
        self.clouds = [0] * len(self.clouds_capacity)
        self.fogs = [0] * len(self.fogs_capacity)
        self.lost_data = 0

    def get_state(self, active_ru_num: int) -> List[float]:
        clouds = [self.clouds[index] / self.clouds_capacity[index] for index in range(len(self.clouds))]
        fogs = [self.fogs[index] / self.fogs_capacity[index] for index in range(len(self.fogs))]
    ...
    def get_reward(self, ru_index: int, active_ru_num: int, action: int) -> float:
        ...
        for cu in self.clouds_capacity:
            if active_ru_num * 1966 < cu:
                fit_cloud = True
        ...
        if ru_index == active_ru_num:
            ...
            if sum(self.fogs) > 0:
                for index in range(len(self.clouds)):
                    percent = min(self.clouds[index] / self.clouds_capacity[index], percent)
    ...
    def alloc_resource(self, split: int, ru_index: int, active_ru_num: int) -> List[Any]:
        ...
        for index in range(len(self.clouds)):
            if self.clouds[index] + cloud_part <= self.clouds_capacity[index]:
                self.clouds[index] += cloud_part
                cloud_part = 0
                break

        if split != 0:
            for index in range(len(self.fogs)):
                if self.fogs[index] + fog_part <= self.fogs_capacity[index]:
                    self.fogs[index] += fog_part
                    fog_part = 0
                    break
...
def train(path: str, arguments: argparse.Namespace, parameters: Dict[str, Any]) -> None:
    ...
    ddqn = DDQN(
        num_input=1 + len(env.clouds) + len(env.fogs),
        num_actions=4,
